...
test('started 无法保存时拒绝文件写入但仍允许只读调用', async t => {
  const f = await fixture(t), ticket = await f.register();
  ...
  const readable = path.join(f.workspace, 'readable.txt');
  writeFileSync(readable, '仍可读取', 'utf8');
  renameSync(journal, savedJournal); mkdirSync(journal);
  let result: Wire, read: Wire;
  try {
    result = await f.call('filesystem_write', { ticket_id: ticket.ticket_id, path: file, content: 'THIS MUST NEVER BE WRITTEN' });
    read = await f.call('filesystem_read', { ticket_id: ticket.ticket_id, path: readable });
  } finally {
  ...
  assert.equal(read.content, '仍可读取');
  assert.equal(read.harness_recording.started, 'recording-failed');
  assert.equal(read.harness_recording.result, 'recording-failed');
});
