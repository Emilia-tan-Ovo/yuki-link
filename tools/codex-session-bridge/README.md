...
元数据写入临时文件并原子替换；事件逐条追加。启动恢复不自动重发未完成请求，标记 `interrupted`，防止重复副作用。若记录的旧 PID 仍存活，启动失败并要求人工核实，不杀死可能已经被复用的 PID。损坏的完整日志或无法解释的锁会保留并报错，不静默重置。单次崩溃留下的半行 JSONL 尾部会被修复。
