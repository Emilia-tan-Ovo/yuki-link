...
import { api } from './api';
import { ConversationRow } from './renderers';
...
export function Conversation({ id, initial, relation, composer, refresh }: { id: string; initial?: ConversationPage; relation: ConversationLink; composer: ComposerCapability; refresh: number }) {
  ...
  async function load(direction: 'before' | 'after') {
    ...
    try {
      ...
      if (scroll.current) {
        if (direction === 'before') anchor.current = captureAnchor(scroll.current);
  ...
  return <><div ref={scroll} className="conversation-scroll" tabIndex={0} role="region" aria-label="Conversation 历史">
    <div className="reading-column conversation-history">
      ...
      {page?.items.map(item => <ConversationRow key={item.id} item={item} />)}
