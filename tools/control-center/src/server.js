...
export function createServer(supervisor, { codexCheck, startup, deploymentCheck, deploymentUpdate } = {}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    if (req.headers.host !== new URL(origin).host || (req.headers.origin && req.headers.origin !== origin)
      || ['cross-site', 'same-site'].includes(req.headers['sec-fetch-site'])) return json(403, { code: 'FORBIDDEN' });
    if (req.method === 'GET' && assets.has(req.url)) {
      const [name, type] = assets.get(req.url);
      res.writeHead(200, { ...headers, 'content-type': type, ...(req.url === '/' ? { 'set-cookie': `yuki_cc=${session}; HttpOnly; SameSite=Strict; Path=/` } : {}) });
    ...
    try {
      ...
      if (req.url === '/api/action') {
        if (Object.keys(body).some(k => !['id', 'action', 'confirm'].includes(k)) || (body.confirm !== undefined && typeof body.confirm !== 'boolean')) return json(400, { code: 'INVALID_ACTION' });
        await supervisor.action(body.id, body.action, body.confirm === true);
      ...
      json(200, { ok: true });
    } catch (e) { json(409, { code: /^[A-Z_]{2,80}$/.test(e.code ?? '') ? e.code : 'ACTION_FAILED' }); }
  });
}
