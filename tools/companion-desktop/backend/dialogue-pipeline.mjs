...
export class DialoguePipeline {
  ...
  async run(text, { roleCard, thinking, runtime, memory = { schemaVersion: 1, entries: [] }, onProviderFailure = () => {} }) {
    ...
    if (reasoningContent && Buffer.byteLength(reasoningContent, 'utf8') > 256 * 1024) {
      let bytes = Buffer.from(reasoningContent).subarray(0, 256 * 1024);
      while (bytes.length && (bytes.at(-1) & 0xc0) === 0x80) bytes = bytes.subarray(0, -1);
      if (bytes.length && bytes.at(-1) >= 0xc0) bytes = bytes.subarray(0, -1);
      reasoningContent = bytes.toString('utf8');
