...
test('runtime capability status changes on provider success and failure only', async t => {
  ...
  const seen = [];
  let fail = false;
  const session = new BackendSession({ directory: dir, provider: async input => { seen.push(runtimeService(input.messages)); if (fail) throw Error('remote failed'); return '好'; } });
