...
const config = { schemaVersion: 1, enabled: true, provider: 'qwen', region: 'cn', asrModel: 'qwen3-asr-flash-2026-02-10', ttsModel: 'qwen3-tts-instruct-flash-2026-01-26', voice: 'Cherry', inputDevice: 'default', outputDevice: 'default' };
...
test('ASR sends only bounded audio and exact pinned request, releases bytes, returns final transcript', async () => {
  ...
  const provider = createVoiceProvider({ config, key: 'fixture-key', fetcher: async (url, options) => { calls.push({ url, options }); return json({ choices: [{ finish_reason: 'stop', message: { content: '  你好  ' } }] }); } });
  assert.deepEqual(await provider.transcribe({ wav: bytes }), { text: '你好', safeUsage: null });
  assert.equal(calls.length, 1); assert.equal(calls[0].url, 'https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions');
  assert.equal(calls[0].options.headers.authorization, 'Bearer fixture-key');
  assert.deepEqual(JSON.parse(calls[0].options.body), { model: config.asrModel, messages: [{ role: 'user', content: [{ type: 'input_audio', input_audio: { data: 'data:audio/wav;base64,' + encoded } }] }], stream: false, asr_options: { enable_itn: false } });
...
test('TTS uses only committed text projection, exact request, signed HTTPS download without auth', async () => {
  ...
  assert.equal(calls.length, 2); assert.equal(calls[0].target, 'https://dashscope.aliyuncs.com/api/v1/services/aigc/multimodal-generation/generation');
  assert.deepEqual(JSON.parse(calls[0].options.body), { model: config.ttsModel, input: { text: '完整正文', voice: 'Cherry', language_type: 'Auto', instructions: '用自然、中性的语气清晰朗读。', optimize_instructions: false } });
...
test('ASR rejects non-final/empty/bad response and classifies HTTP errors without raw data or retry', async () => {
  for (const [response, code] of [[json({ choices: [] }), 'INVALID_RESPONSE'], [json({ choices: [{ finish_reason: 'length', message: { content: 'x' } }] }), 'INVALID_RESPONSE'], [json({ choices: [{ finish_reason: 'stop', message: { content: ' ' } }] }), 'NO_SPEECH'], [new Response('private body'), 'INVALID_RESPONSE'], [new Response('private body', { status: 401 }), 'AUTH'], [new Response('private body', { status: 429 }), 'RATE_LIMIT'], [json({ code: 'private' }), 'PROVIDER_FAILED']]) {
