...
async function privateDirectory(directory, action) {
  if (process.platform !== 'win32') throw Error('VOICE_STORAGE_UNAVAILABLE');
  await run('pwsh.exe', ['-NoProfile', '-NonInteractive', '-File', fileURLToPath(new URL('./private-voice-directory.ps1', import.meta.url)), '-Target', directory, '-Action', action], { windowsHide: true, timeout: 10000 });
}
// Secret and private paths never leave main except the key sent to its own worker.
export class VoiceCredentialStore {
  constructor(directory, safeStorage, protect = privateDirectory) { this.directory = join(directory, 'voice-private'); this.file = join(this.directory, 'voice-credential.bin'); this.crypto = safeStorage; this.protect = protect; this.queue = Promise.resolve(); }
  async read() {
    try {
