...
export class BackendConnection {
  constructor({ worker, onMessage }) { this.worker = worker; this.onMessage = onMessage; this.generation = 0; this.child = null; this.state = 'disconnected'; }
  start(config) {
    this.close();
    const generation = ++this.generation;
    this.state = 'connecting';
    const child = utilityProcess.fork(this.worker, [], { stdio: 'ignore', serviceName: 'Yuki Link Text Backend' });
    this.child = child;
  ...
  send(message, generation) {
    ...
    this.child.postMessage(message); return true;
  }
  close() {
