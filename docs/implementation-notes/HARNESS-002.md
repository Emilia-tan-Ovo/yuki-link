...
- 验证：新增用例定向 1/1 pass，exit 0；`node --test test/shutdown.test.js` **3/3 pass，exit 0**（SPEC-1 修复所在 Codex implementation session 内执行）；`npm run typecheck` **exit 0**；`git diff --check` exit 0。日志：`.local/workflow-state/HARNESS-002-SPEC-1-red.log`、`HARNESS-002-SPEC-1-targeted.log`、`HARNESS-002-SPEC-1-shutdown.log`、`HARNESS-002-SPEC-1-typecheck.log`。
